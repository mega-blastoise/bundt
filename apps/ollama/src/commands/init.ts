...
# Generated by: ollama-bundt init
#
# Prerequisites:
...
services:
  ...
  ollama:
    ...
    volumes:
      ...
      # Model Storage
      # ---------------------------------------------------------------------------
      # Maps ~/.ollama-bundt/models on your host to /root/.ollama in the container
...
# Generated by: ollama-bundt init --no-gpu
#
# Performance notes:
...
run = "ollama-bundt docker-up"

[tasks.down]
description = "Stop all containers"
run = "ollama-bundt docker-down"

[tasks.restart]
description = "Restart all containers"
run = "ollama-bundt docker-restart"

[tasks.status]
description = "Show container status"
run = "ollama-bundt docker-status"

[tasks.logs]
description = "Follow container logs"
run = "ollama-bundt docker-logs --follow"
...
run = "ollama-bundt config"
`;

// README.md template with comprehensive documentation
const README_TEMPLATE = `# Project Name

> Powered by [ollama-bundt](https://github.com/nicholasgalante1997/ollama-bundt) - Run Claude Code with local Ollama LLMs

## Prerequisites

- [Docker](https://docs.docker.com/get-docker/) with Docker Compose v2
- [mise](https://mise.jdx.dev/) task runner (recommended)
- [ollama-bundt](https://www.npmjs.com/package/ollama-bundt) CLI (\`npm install -g ollama-bundt\`)
...
### Loclaude Config (\`.ollama-bundt/config.json\`)
...
This project uses [ollama-bundt](https://github.com/nicholasgalante1997/ollama-bundt) to run Claude Code with local Ollama LLMs.
...
- **Loclaude:** \`.ollama-bundt/config.json\`
- **Tasks:** \`mise.toml\`
...
export async function init(options: InitOptions = {}): Promise<void> {
  ...
  const configDir = join(cwd, '.ollama-bundt');
  const configPath = join(configDir, 'config.json');
  const globalModelsDir = join(homedir(), '.ollama-bundt', 'models');
  ...
  header('Initializing ollama-bundt project');
  console.log('');
  ...
  // Create .ollama-bundt config directory
  if (!existsSync(configDir)) {
    mkdirSync(configDir, { recursive: true });
    console.log(success(`Created ${file('.ollama-bundt/')} directory`));
  }

  // Create config file
  if (existsSync(configPath) && !options.force) {
    console.log(warn(`${file('.ollama-bundt/config.json')} already exists`));
  } else {
    writeFileSync(configPath, getConfigTemplate(gpuMode, modelsPath));
    console.log(success(`Created ${file('.ollama-bundt/config.json')}`));
  }
